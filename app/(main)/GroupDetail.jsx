...
const GroupDetail = () => {
    ...
    const handleMembersData = (data) => {
        ...
        if (members[0]?.userId) {
            ...
            const currentUserMember = members.find(m =>
                m.userId && m.userId._id === user._id
            );
        ...
        if (conversation && !initializedRef.current) {
            ...
            if (conversation.members) {
                ...
                if (conversation.members[0]?.userId) {
                    ...
                    // Kiểm tra xem user hiện tại có phải là admin của nhóm không
                    const currentUserMember = conversation.members.find(m =>
                        m.userId && m.userId._id === user._id
                    );
                    setIsAdmin(currentUserMember?.role === 'admin');
                } else {
    ...
    useEffect(() => {
        ...
        socket.on('member_role_updated', ({ conversationId, targetUserId, newRole, updatedConversation }) => {
    ...
    const fetchFriends = async () => {
        ...
        try {
            ...
            if (response.data && response.data.acceptedFriends) {
                ...
                const existingMemberIds = members.map(member => member?._id?.toString()).filter(Boolean);
            ...
            console.error('Error fetching friends:', error.message || 'Unknown error');
    ...
    const toggleSelectFriend = (friend) => {
        ...
        setSelectedFriends(prevSelected => {
            ...
            const alreadySelected = prevSelected.some(f => f._id && f._id.toString() === friendId);

            if (alreadySelected) {
                // Loại bỏ khỏi danh sách đã chọn
                return prevSelected.filter(f => f._id && f._id.toString() !== friendId);
            } else {
    ...
    const confirmAddMembers = () => {
        ...
        setMembers(prevMembers => [...prevMembers, ...newMembers.map(friend => ({
            ...friend,
            role: 'member'
        }))]);
    ...
    const handleRemoveMember = (memberId) => {
        ...
        const isTargetAdmin = members.find(m => m._id === memberId)?.role === 'admin';
        const adminCount = members.filter(m => m.role === 'admin').length;
    ...
    const handleUpdateRole = (newRole) => {
        ...
        // Hiển thị thông báo
        const actionText = newRole === 'admin' ? 'thăng cấp thành admin' : 'hạ cấp xuống thành viên thường';
        Alert.alert('Thành công', `Đã ${actionText} cho ${selectedMember.username}`);
                ...
                >
                    <View style={styles.modalContainer}>
                        <View style={styles.modalContent}>
                            ...
                            >
                                <MaterialIcons name="admin-panel-settings" size={24} color="#297EFF" />
                                <View style={styles.roleOptionContent}>
                                    <Text style={styles.roleOptionTitle}>Quản trị viên</Text>
                                    <Text style={styles.roleOptionDesc}>Có thể quản lý thành viên và cài đặt nhóm</Text>
                                </View>
                            </TouchableOpacity>
                            ...
                            >
                                ...
                                <View style={styles.roleOptionContent}>
                                    ...
                                    <Text style={styles.roleOptionDesc}>Chỉ có thể nhắn tin và xem thông tin nhóm</Text>
                                </View>
                            </TouchableOpacity>
                            ...
                            <View style={styles.modalButtonContainer}>
                                ...
                                >
                                    {isUpdating ? (
                                        ...
                                        <Text style={[styles.modalButtonText, styles.confirmButtonText]}>Lưu</Text>
                                    )}
                                </TouchableOpacity>
                            </View>
                        </View>
                        ...
                        {isLoadingFriends ? (
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => {
                                    ...
                                    const isSelected = selectedFriends.some(f => f._id && f._id.toString() === friendId);
                                    ...
                                    return (
                                        ...
                                        >
                                            <View style={styles.friendInfo}>
                                                ...
                                                />
                                                <View>
                                                    <Text style={styles.friendName}>{item.username || 'Người dùng'}</Text>
                                                    {item.phoneNumber && (
                            ...
                            <View style={styles.noResultsContainer}>
                                {searchText ? (
                                    <Text style={styles.noResultsText}>Không tìm thấy bạn bè phù hợp</Text>
                                ) : friendsList.length === 0 ? (
                                    <View>
                                        <Text style={styles.noResultsText}>Bạn chưa có bạn bè nào</Text>
                                        <Text style={[styles.noResultsText, { marginTop: 10, fontSize: 14 }]}>
                                            Thêm bạn bè trước khi mời họ vào nhóm
                                        </Text>
                                    </View>
                                ) : (
                                    <Text style={styles.noResultsText}>Tất cả bạn bè đã có trong nhóm</Text>
                                )}
                            </View>
                        )}
